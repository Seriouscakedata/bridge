...
🔍 ВЕРИФИКАЦИЯ ИМЁН СИМВОЛОВ ПЕРЕД severity=serious (undefined-symbol):
Если ты считаешь, что вызываемая функция/командлет «не существует» (undefined-symbol) — ОБЯЗАТЕЛЬНО до выставления severity=serious пройди мысленный grep по трём источникам:
1. Строки DIFF ниже (со знаком + или -) — есть ли ТОЧНОЕ имя (посимвольно) хотя бы в одной строке?
2. Блок SYMBOL_EVIDENCE ниже — там собраны сигнатуры функций из смежных файлов.
3. Блок HEAD-контекст ниже — там определения функций из изменённых файлов.
Правило: если имя найдено хотя бы в одном источнике — НЕ флагируй как undefined. Если найдено ПОХОЖЕЕ имя (отличается префиксом/суффиксом/опечаткой) — это minor-замечание «возможная опечатка», но НЕ severity=serious. Если ты «помнишь» имя из тренировочных данных без его подтверждения в diff/SYMBOL_EVIDENCE/HEAD — это hallucination; не используй его как основание для serious-флага.
