...
### Автопилот: само-продвижение по главам и само-лечение зависших задач
**Продвижение по главам (детерминированное, не на глаз).** Раньше координатор (project-autopilot)
сам, на усмотрение LLM, решал «какая глава следующая / релиз ли уже готов» — и на многоглавом проекте
мог консервативно решить «готово» раньше времени, выдать 3 пустых прогона подряд и уйти в паузу
(`emptyCoordinatorLimit=3`), оставив проект недостроенным (наблюдалось: стоп на 3/8 глав). Теперь
`New-ProjectAutopilotCoordinatorTaskText` (lib/backlog-autopilot.ps1) детерминированно считает прогресс
из `PROJECT_PLAN.md` (заголовки `## Глава N`) + живого backlog (различные `chapter`-слаги уже
заведённых атомов) и вшивает в задание координатору блок «DETERMINISTIC PLAN PROGRESS»: сколько глав
всего, какие сделаны, и какую главу декомпозировать СЛЕДУЮЩЕЙ — с запретом завершать релиз, пока главы
остаются. Итог: автопилот ведёт проект по плану до конца сам. Если очередь реально пуста и все главы
сделаны — он завершает штатно.

**Само-лечение зависших дубликатов.** `Invoke-ProjectAutopilotHeldDuplicateAutoHeal` (вызов в
`Start-ProjectAutopilotIfNeeded` перед решением о постановке coordinator) авто-закрывает задачу,
застрявшую в `held` с `attempts=0` (заheld до запуска — дедупом/touch-overlap), КОГДА её deliverable
уже готов: все файлы из поля `files` существуют И закоммичены в git проекта. Жёсткие предохранители
(только не начинавшиеся задачи, только project-scope, не operator-held/failed) исключают авто-зачёт
настоящей недоделки. Сообщение в канал: «🩹 авто-закрыто N зависших дубликат-задач».
