  ...
  foreach ($sp in $candidates) {
    ...
    try {
      ...
      if ($st.heartbeat) {
        ...
        if ($age -lt 1800) { $hbOk = $true; break }   # 2026-06-22: 1200->1800s (operator harden: heavy DONE-gate cycle grew past 20min, watchdog.log showed driver hbAge 1240-1600s -> false driver-restart that interrupted finalization; real root = the heavy gate-regression/smoke SUBPROCESS doesn't pump the driver heartbeat while the driver blocks on it — bridge's approved gate-regression timeout-tuning shortens that cycle). 2026-06-20: 300s->1200s (operator directive: add time where a timeout kills a WORKING process). The heavy DONE-gate cycle (parse 307 ps1 + smoke + gate-regression, ~12min) runs in the driver loop WITHOUT pumping the heartbeat, so a HEALTHY-but-finalizing driver looked stale past 5min -> UNHEALTHY -> recycle interrupted finalization before it could close the task (self-reinforcing loop, ATOM_006). 20min tolerance covers the real cycle; a genuinely dead engine is still caught.
      }
    } catch {}
  }
