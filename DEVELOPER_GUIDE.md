...
Task Scheduler (autostart, elevated)
        └── supervisor.ps1            # следит, перезапускает, circuit-breaker
              ...
              ├── driver.ps1 -Channel sample-project / another-project / ... # по одному driver на активный канал
...
внешним проектам (`sample-project`, `another-project`, ...). У каждого свой `channels/<slug>/`:
...
```
Агенты должны вызывать это через `[[RUNJOB: ... | <bridge-root>]]`, а не собирать inline `Start-Process npm run dev`.
