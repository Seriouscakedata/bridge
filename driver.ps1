...
$script:CuratorLibPath = Join-Path (Get-BridgeRoot) 'lib\backlog.ps1'
$script:CuratorControlPath = Join-Path (Get-BridgeRoot) 'control'
...
@"
- САМОУЛУЧШЕНИЕ РАЗРЕШЕНО: тебе МОЖНО улучшать сам мост (файлы в `$bridgeRoot`: `web\index.html`, `server.ps1`, `driver.ps1`, `lib\common.ps1` и т.п.). СТРОГИЕ ПРАВИЛА БЕЗОПАСНОСТИ (нарушение убьёт мост):
  ...
  3) Применить правки .ps1-файлов движка: создай файл `control\restart.flag` в корне моста -- супервизор перезапустит мост (без UAC). ⛔ СТРОГИЙ ЗАПРЕТ: restart.flag создавать ТОЛЬКО если изменён хотя бы один `.ps1`-файл. Перед созданием флага ОБЯЗАТЕЛЬНО проверь: `git -C "$bridgeRoot" diff --name-only HEAD` -- если в выводе НЕТ ни одного `.ps1`, флаг НЕ создавай (мост перезапустится зря).
  4) После КАЖДОЙ проверенной рабочей правки: `git -C "$bridgeRoot" add -A; git -C "$bridgeRoot" commit -m "что сделал"`. Это фиксирует прогресс (watchdog откатит на последний коммит при поломке).
...
🖼 ПРАВИЛО ВИЗУАЛЬНОЙ БАЗЫ (UI-задачи): до первой правки HTML/CSS/JS — ОБЯЗАТЕЛЬНО сними baseline-скрин в обоих вьюпортах. Используй [[RUNJOB: powershell -NoProfile -ExecutionPolicy Bypass -File tools\visit.ps1 -Url http://localhost:8787 | $bridgeRoot]] (desktop 1920×1080) и [[RUNJOB: powershell -NoProfile -ExecutionPolicy Bypass -File tools\visit.ps1 -Url http://localhost:8787 -Mobile | $bridgeRoot]] (mobile 390×844). Получив пути PNG — отправь планировщику [[FILE: путь]] оба снимка. «Diff выглядит правильно» без визуального снимка не аргумент. Если задача про ВНЕШНИЙ сайт — замени URL в RUNJOB на нужный.
