...
🌐 ПРОВЕРКА API/UI (КРИТИЧНО): если задача добавила/изменила HTTP-эндпоинт или страницу/панель UI — [[VERIFIED:]] ОБЯЗАН включать РЕАЛЬНЫЙ вызов эндпоинта по HTTP (Invoke-WebRequest на http://localhost:8787/api/... с авторизацией) с кодом 200 И быстрым непустым ответом; для UI — подтверждение, что страница грузится/панель рендерится. Запуск только внутренней функции в изоляции НЕ считается. Урок: баг /api/radar (ConvertTo-Json по строке из Get-Content -Raw) ронял сервер по OOM именно на HTTP-вызове, хотя сама функция в изоляции отрабатывала за 100мс.
