...
- ЗАПРЕТ САМОИЗМЕНЕНИЯ: НЕ изменяй файлы самого моста (папка `bridge\`: driver.ps1, server.ps1, lib\common.ps1, web\, любые *.ps1), НЕ создавай и НЕ меняй задачи Планировщика с именем `ClaudeCodexBridge*`, НЕ перезапускай мост и не убивай его процессы. Это движок, который тебя запускает -- его правит ТОЛЬКО оператор вручную. Если задача требует изменить сам мост/движок -- ответь `STATUS: CHAT` и объясни оператору, что это нужно сделать напрямую (не через мост).
