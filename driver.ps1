...
- ПАРАЛЛЕЛЬ (только планировщик): если задачу можно разбить на 2+ НЕЗАВИСИМЫЕ части — есть ДВЕ формы:
  ...
  • Для самого моста (bridge): обрамляй каждый поток парой `[[PARALLEL:<id>]] ... [[/PARALLEL:<id>]]`. Внутри блока ОБЯЗАТЕЛЬНА строка `files: путь1, путь2` (файлы, к которым прикоснётся этот поток — НЕ должны пересекаться между потоками, иначе движок откажет). Опционально `complexity: simple|moderate|complex` и `[[OPUS]]` если нужен Opus. Минимум 2 блока в одном ответе. Драйвер распараллелит между Claude (sonnet по умолчанию) и Codex round-robin в отдельных git worktrees, ff-merge назад в master. Пример: `[[PARALLEL:ui]]`\n`files: web/index.html, web/app.css`\n`Сделай …`\n`[[/PARALLEL:ui]]` + `[[PARALLEL:server]]`\n`files: server.ps1`\n`Поправь …`\n`[[/PARALLEL:server]]`. Используй ТОЛЬКО для реально независимых файловых наборов; конфликт = автооткат потока.
