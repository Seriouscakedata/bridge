...
> **[PROTECTED SERVICE] ЛАПА (computer-control)** — GUI-руки оператора для задач на ПК. Вызов: `[[ЛАПА: skill | key=val]]`. Навыки: `open-app`, `type`, `telegram-send`, `telegram-send-sticker`, `operator-task`. `operator-task` — новый planner/LLM слой над руками ЛАПЫ: `goal=<задача>`, опционально `allowed_apps`, `allowed_skills`, `forbidden_actions`, `max_steps`; risky-действия без подтверждения откажет. Точка входа: маркер `[[ЛАПА]]` → `tools/lapa-control.ps1` (`Invoke-LapaSkill`) → `python -m lapa.cli` с cwd = `C:\Users\rafie\bridge-projects\lapa` (overridable `$env:LAPA_ROOT`). Пакет лапы: `bridge-projects/lapa/lapa/` (`cli.py`, `operator.py`, `resolver.py`, `selector.py`, `vision.py`, `act.py`, `verifier.py`). Старый `cc.py` (в `bridge-projects/computer-control/`) — отдельный legacy-проект, лапа его заменила; `tools/computer-use.ps1` — отдельный инструмент, не точка входа лапы. Архитектура: Operator/Planner → Resolver→Selector(UIAutomation→Vision)→Act→Verifier→Reporter. Гейты: verifier ≥0.40 (confidence), safety/risk ladder, STOP. Используй для GUI/ПК задач, НЕ для файлов/git.
