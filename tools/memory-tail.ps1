...
# The driver now fires this script as a detached process (NOT a bridge job: active_jobs are
# awaited by preflight, which would re-block the loop) and closes immediately; memory lands a
# few seconds later and reports to the channel like before. Payload JSON keeps the big strings
# out of the command line; it is deleted on completion.
...
try {
  ...
  try {
    ...
    if ($memId) { [void]$notes.Add('память') }
  } catch {}
  try {
  ...
  try {
    if ($taskTurn -ge 2 -and $didActions -and $modeBefore -ne 'study') {
      ...
      if ($skillId) { [void]$notes.Add('плейбук') }
    }
  } catch {}

  if ($notes.Count -gt 0) {
    try { Add-Message -From system -Text ('🧠 Память дописана в фоне: ' + ($notes -join ' + ') + '.') -Kind event | Out-Null } catch {}
  }
} finally {
