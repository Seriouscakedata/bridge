...
function Invoke-CodexSecurityPass {
  ...
  [void]$bodyBuilder.AppendLine('- command injection: где user-input (HTTP body, URL params, file content) попадает в shell/Invoke-Expression без sanitization')
  ...
  [void]$bodyBuilder.AppendLine('- audit-security.ps1: использует Invoke-Expression намеренно для анализа')
