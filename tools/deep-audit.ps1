...
function Complete-CodexSecurityAsync {
  ...
  if (-not $waited) {
    Stop-DeepAuditProcessTree -Proc $State.proc
...
function Invoke-MultiAgentDeepAudit {
  ...
  # Q5: required slices get a higher timeout than optional
  ...
  $requiredTimeoutSec = [int]([Math]::Ceiling($TimeoutSec * $requiredMultiplier))
  ...
  while ($queue.Count -gt 0 -or $running.Count -gt 0) {
    ...
    while ($queue.Count -gt 0 -and $running.Count -lt $maxPar) {
      ...
      try {
        ...
        $agentTimeout = if ($requiredSlices -contains $spec.role) { $requiredTimeoutSec } else { $TimeoutSec }
        [void]$running.Add((Start-DeepAuditAgentProcess -Spec $spec -BridgeRoot $BridgeRoot -ProjRoot $ProjRoot -TimeoutSec $agentTimeout -RunLLM $RunLLM))
      } catch {
