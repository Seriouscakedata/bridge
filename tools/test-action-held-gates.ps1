...
Check '83 retry guard allows only covered verified DONE evidence as zero-diff evidence' ($agentTurnSource -notmatch 'Test-TaskActionEvidenceBypassMarker' -and $agentTurnSource -match 'Test-TaskCoveredVerifiedDoneEvidence\s+-Reply\s+\$reply' -and $agentTurnSource -match 'codex_evidence_retry_count\s+-NotePropertyValue\s+0') $agentTurnSource
