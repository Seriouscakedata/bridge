...
// @audit-safe: yes (only GET /api/features, no mutations)
...
async function scenario(s) {
  ...
  let listResp = null;
  try {
    const r = await scenarioFetch('/api/features');
    if (!r.ok) { s.fail('GET /api/features HTTP ' + r.status); return; }
    listResp = await r.json();
  } catch (e) { s.fail('GET failed: ' + e.message); return; }

  const features = (listResp && Array.isArray(listResp.features)) ? listResp.features
                 : (listResp && Array.isArray(listResp.items)) ? listResp.items
  ...
  let detailResp = null;
  try {
    const r = await scenarioFetch('/api/features/auditor');
