...
'@
  ...
  Check 'Gate regression inconclusive runtime: plannerStatus stays DONE (non-blocking)' ($plannerStatus -eq 'DONE') $plannerStatus
  Check 'Gate regression inconclusive runtime: gate_regression_failed is cleared by handler' ((@($script:ClearedGateFailureKinds) -contains 'gate_regression_failed')) $script:ClearedGateFailureKinds
  Check 'Gate regression inconclusive runtime: state records inconclusive outcome' ([string]$script:FakeGateState.gate_regression_last_outcome -eq 'inconclusive' -and [int]$script:FakeGateState.gate_regression_last_attempts -eq 4 -and (@($script:FakeGateState.gate_regression_last_budgets) -join ',') -eq '60,600,60,120') $script:FakeGateState
  Check 'Gate regression inconclusive runtime: event says inconclusive outcome with fallback' ($timeoutFailMessage.Contains('inconclusive') -and $timeoutFailMessage.Contains('budgets=60,600,60,120') -and $timeoutFailMessage.Contains('fallback=600s')) $timeoutFailMessage
  ...
  Check 'Gate regression inconclusive runtime: handler returns inconclusive outcome' ([string]$timeoutFailRecord.Outcome -eq 'inconclusive') $timeoutFailRecord
