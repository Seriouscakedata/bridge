...
function Invoke-BacklogCurator {
  ...
  try {
    ...
    # Kill-switch (operator emergency off, default ON). The flag lives in the protected
    # store (~/.bridge-private), which the coder sandbox cannot write -- so the bridge can
    # never silently re-enable its own autonomous approval if the operator turned it off.
    # File ABSENT (default) => autonomous approval active. File PRESENT => OFF: every idea
    # routes to the operator (no auto-approve, no auto-reject), i.e. the old manual flow.
    $killSwitchOff = $false
    try {
      if (Get-Command Get-PrivateFilePath -ErrorAction SilentlyContinue) {
        $ksp = Get-PrivateFilePath 'curator-autoapprove.off'
        if ($ksp -and (Test-Path -LiteralPath $ksp)) { $killSwitchOff = $true }
      }
    } catch { $killSwitchOff = $false }
    if ($killSwitchOff) {
      $ksRisk = [pscustomobject]@{ operator_required=$true; auto_approve_allowed=$false; risk='killswitch'; category='killswitch-off'; reason='curator-autoapprove.off present'; evidence=@('autonomous-approval kill-switch file present') }
      return (Set-BacklogCuratorDecision -Items $items -Item $item -Decision 'operator-required' -Confidence 1.0 -Reason 'autonomous-approval kill-switch is OFF (curator-autoapprove.off present); operator approval required' -PolicyRisk $ksRisk -Rubric $null -Model 'killswitch-v1')
    }
