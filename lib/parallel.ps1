...
Жёсткие правила:
- Меняй ТОЛЬКО файлы из списка «Разрешённые файлы» ниже — это исчерпывающий список. Любой другой файл (в т.ч. общий: модель, конфиг, build-файл) принадлежит ДРУГОМУ атому/потоку — НЕ трогай его, даже если кажется, что он нужен. Если без чужого файла никак — заверши STATUS: FAILED и в первой строке отчёта укажи `needs-shared-file: <путь>`, не трогая его.
- Путь и имя файла используй ТОЧНО как в списке (тот же регистр). Не переименовывай и не создавай дубликат под другим регистром.
...
- Меняй ТОЛЬКО файлы из списка «Разрешённые файлы» ниже (исчерпывающий список). Чужие/общие файлы НЕ трогай; если без чужого файла никак — STATUS: FAILED и в первой строке `needs-shared-file: <путь>`. Путь и регистр — точно как в списке.
- Не трогай файлы вне текущего worktree.
...
function Stop-ParallelDispatchStalledWorker {
  ...
  try {
    $proc = $null
    try { $proc = $Worker.process } catch {}

    $workerPid = 0
    try { $workerPid = [int]$Worker.pid } catch {}
