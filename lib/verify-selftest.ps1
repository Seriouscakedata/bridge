...
function Invoke-GateRegressionSuite {
  ...
  # The caller-owned TimeoutSec is the hard suite budget. Scoped selection narrows which tests
  # run, but it must not expand the DONE-gate cap (currently fixed 120s x 2 attempts), otherwise
  # stable regressions turn into long waits before the timeout_fail policy can act.
  $perTestTimeout = if ($selectedTests.Count -gt 0) { 60 } else { [int]$TimeoutSec }
  $wrapperTimeout = [Math]::Max(1, [int]$TimeoutSec)
