    ...
    if (Test-Path -LiteralPath $planFileCP -PathType Leaf) {
      ...
      if ($totalChCP -gt 0) {
        ...
        $nextChapterDirectiveCP = if ($diffMode -eq 'wide' -or $diffMode -eq 'diffusion') {
          "CHAPTERS REMAINING: Chapter $nextNCP through Chapter $totalChCP -- you MUST decompose EVERY one of these $remainCP chapters into atoms in THIS SINGLE run. A PROJECT_BACKLOG containing only Chapter $nextNCP (or any single chapter) is a FAILURE in this mode; the whole point is ONE wide backlog so INDEPENDENT atoms across all remaining chapters run in PARALLEL. Independent screens/modules get an EMPTY depends_on (they run concurrently in the first wave); only a genuinely dependent chapter gets depends_on (integration needs the screens; the release/APK needs integration)."
        } else {
