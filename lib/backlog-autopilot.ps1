    ...
    if (Test-Path -LiteralPath $planFileCP -PathType Leaf) {
      ...
      if ($totalChCP -gt 0) {
        ...
        $decomposeScopeLine = if ($diffMode -eq 'wide' -or $diffMode -eq 'diffusion') {
          ...
          "Decompose ALL remaining approved chapters (Chapter $nextNCP through Chapter $totalChCP) into atoms in ONE PROJECT_BACKLOG this run (cross-chapter, all-chapters decomposition). Give INDEPENDENT atoms (different files, no real prerequisite) an EMPTY depends_on so they run in parallel across chapters; give DEPENDENT atoms an explicit depends_on on the prerequisite atom's slug (those serialize unless a contract lets them parallelize). $contractClauseCP DECOMPOSE BY COHERENT UNIT -- this is the single most important rule for CLEAN parallelism: one atom = ONE self-contained feature/screen/module WITH ALL its tightly-coupled files listed in that atom's files (a screen atom OWNS its screen + its ViewModel/state + its preview + its formatting; a domain atom OWNS its model + mapper + helpers). Do NOT split a single screen's coupled files across SEPARATE atoms -- a coder writing a screen naturally writes its state and preview in the SAME turn, so separate one-file atoms COLLIDE on each other's files and get quarantined for changing paths outside their declared set (this is the #1 cause of wasted parallel work, observed live: 13 of 17 streams quarantined). Every INDEPENDENT atom MUST own a DISJOINT file set (no two independent atoms share or sit under the same feature folder). Split by FEATURE, not by file: one atom per screen (with its state+preview), one per domain component group, one per leaf module (theme, resources, navigation, di). Aim for AS MANY independent atoms as there are distinct features/screens/modules (typically 8-16 for a full app) so they run concurrently in the first wave, each owning its OWN files. Only put a depends_on when the feature literally cannot be built before the prerequisite exists (a test needs the impl; the nav-host/integration needs the screens; the release/APK needs integration). More independent COHERENT atoms with DISJOINT files = wider CLEAN parallelism; over-splitting one feature into many one-file atoms does the OPPOSITE -- they collide and serialize."
        } else {
          "Decompose ONLY Chapter $nextNCP into atoms this run."
        }
        # 2026-07-01 planner-compliance fix: in wide/diffusion the singular "NEXT chapter to decompose NOW"
        # anchor made the planner emit ONE chapter per run (observed live: chapters 1,2,3 serial, peak
        # parallelism 2) despite the all-chapters scope line. Replace the singular anchor with an unmissable
        # ALL-remaining-chapters directive so one wide backlog spans every remaining chapter -> real cross-
        # chapter parallelism. 'off'/'shadow' keep the singular one-chapter anchor.
        $nextChapterDirectiveCP = if ($diffMode -eq 'wide' -or $diffMode -eq 'diffusion') {
          "CHAPTERS REMAINING: Chapter $nextNCP through Chapter $totalChCP -- you MUST decompose EVERY one of these $remainCP chapters into atoms in THIS SINGLE run. A PROJECT_BACKLOG containing only Chapter $nextNCP (or any single chapter) is a FAILURE in this mode; the whole point is ONE wide backlog so INDEPENDENT atoms across all remaining chapters run in PARALLEL. Independent screens/modules get an EMPTY depends_on (they run concurrently in the first wave); only a genuinely dependent chapter gets depends_on (integration needs the screens; the release/APK needs integration). CRITICAL -- do NOT defer any chapter to a later coordinator run just because its prerequisite is not built yet: emit the COMPLETE project graph NOW. A chapter whose work needs an earlier chapter (e.g. a screen needs the scaffold) is STILL emitted in THIS backlog, with an explicit depends_on on the prerequisite atom's slug -- the scheduler makes it wait and then runs it automatically, in parallel with its siblings. Emitting only the scaffold/first chapter and leaving the screens for 'next time' is EXACTLY the failure this mode forbids; lay out all $remainCP chapters as one dependency graph right now."
        } else {
          "NEXT chapter to decompose NOW: Chapter $nextNCP - $nextTitleCP"
        }
        if ($nextNCP -le $totalChCP) {
          $chapterProgressBlock = "DETERMINISTIC PLAN PROGRESS (authoritative -- computed from PROJECT_PLAN.md + the live backlog; trust this over your own judgment about whether the release is finished):`n- The approved release IS the full plan: $totalChCP chapters total, ALL approved and in scope. Chapters you have not reached yet are NOT future/optional/out-of-scope work.`n- Full approved plan chapters:`n$($planLinesCP -join "`n")`n- Chapter areas already decomposed into the backlog ($decCountCP of $totalChCP done): $doneSlugsCP`n- $nextChapterDirectiveCP`n- $decomposeScopeLine Do NOT conclude the release is complete -- $remainCP chapter(s) still remain. Do NOT emit a release-scope open-question for Chapter $nextNCP; it is already authorized by the approved plan.`n- Efficiency: the plan progress above is authoritative for scope/status, so you do NOT need to re-read PROJECT_MAP.md or re-scan the whole codebase to decide what is done. Read only the specific source files your Chapter $nextNCP atoms will create, touch, or depend on (check earlier CHAPTER_*_ATOMS.md only for file-ownership of prior chapters to avoid conflicts).`n`n"
        } else {
...
- $(if ($diffMode -eq 'wide' -or $diffMode -eq 'diffusion') { "In THIS run you decompose ALL remaining chapters AT ONCE into one PROJECT_BACKLOG (see DETERMINISTIC PLAN PROGRESS above) -- NOT one chapter. Emit every remaining chapter's atoms now (independent atoms get empty depends_on and run in parallel; dependent ones get depends_on). A single-chapter backlog is WRONG in this mode. Prefer $diffK-$max atoms across the remaining chapters." } else { "Default decomposition is ONE next chapter/wave into small atomic implementation tasks. Prefer 3-$max tasks; fewer is OK if the chapter is small." })
