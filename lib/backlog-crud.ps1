...
function Reopen-BacklogDoneAfterRegression {
  param(
    [Parameter(Mandatory=$true)][string[]]$Ids,
    [Parameter(Mandatory=$false)]$RegressionEvidence,
    [Parameter(Mandatory=$false)][string]$Reason = 'regression-after-done'
  )

  if (-not (Get-Command Test-BacklogGovernorDoneRegressionReopen -ErrorAction SilentlyContinue)) {
    $governorPath = Join-Path $PSScriptRoot 'backlog-governor.ps1'
    if (Test-Path -LiteralPath $governorPath) { . $governorPath }
  }
  if (-not (Get-Command Test-BacklogGovernorDoneRegressionReopen -ErrorAction SilentlyContinue)) {
    return [pscustomobject][ordered]@{ ok = $false; reason = 'governor_reopen_predicate_missing'; reopened_ids = @(); skipped_ids = @($Ids) }
  }
  ...
  if ($idsClean.Count -eq 0) {
    return [pscustomobject][ordered]@{ ok = $true; reason = 'no_ids'; reopened_ids = @(); skipped_ids = @() }
  }

  $getBacklogFn = ${function:Get-Backlog}
  $saveBacklogFn = ${function:Save-Backlog}
  $testReopenFn = ${function:Test-BacklogGovernorDoneRegressionReopen}
  $setPropFn = ${function:Set-BacklogObjectProperty}
  return (Invoke-BacklogLocked ({
    $items = @(& $getBacklogFn)
    $reopened = New-Object 'System.Collections.Generic.List[string]'
    $skipped = New-Object 'System.Collections.Generic.List[string]'
    $dirty = $false
    foreach ($id in @($idsClean)) {
      $item = $null
      foreach ($candidate in @($items)) {
        if ([string]$candidate.id -eq [string]$id) { $item = $candidate; break }
      }
      if ($null -eq $item) {
        [void]$skipped.Add([string]$id)
        continue
      }
      $verdict = & $testReopenFn -Item $item -RegressionEvidence $RegressionEvidence
      if (-not ($verdict -and [bool]$verdict.reopen)) {
        [void]$skipped.Add([string]$id)
        continue
      }

      $previousDoneSha = [string](Get-BacklogGovernorObjectValue -Object $item -Names @('done_sha') -Default '')
      & $setPropFn -Item $item -Name 'status' -Value 'approved'
      & $setPropFn -Item $item -Name 'reopened_after_done_at' -Value ((Get-Date).ToUniversalTime().ToString('o'))
      & $setPropFn -Item $item -Name 'reopened_after_done_reason' -Value ([string]$Reason)
      & $setPropFn -Item $item -Name 'reopened_after_done_evidence' -Value $RegressionEvidence
      & $setPropFn -Item $item -Name 'reopened_after_done_previous_sha' -Value $previousDoneSha
      & $setPropFn -Item $item -Name 'governor_reopen' -Value $verdict
      [void]$reopened.Add([string]$id)
      $dirty = $true
    }
    if ($dirty) { & $saveBacklogFn $items }
    return [pscustomobject][ordered]@{
      ok = $true
      reason = $(if ($reopened.Count -gt 0) { 'done_regression_reopened' } else { 'nothing_to_reopen' })
      reopened_ids = @($reopened.ToArray())
